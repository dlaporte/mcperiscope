...
export function ContextModal({ evalIndex, totalTokens, onClose }: Props) {
  // Last fetch result, tagged with the eval it was for (loading until it matches)
  const [fetched, setFetched] = useState<{ evalIndex: number; context: ContextData | null; error: string | null } | null>(null);
  const loading = fetched?.evalIndex !== evalIndex;
  const context = loading ? null : fetched?.context ?? null;
  ...
  useEffect(() => {
    ...
    api.getEvalContext(evalIndex)
      .then((data: ContextData) => { if (!cancelled) setFetched({ evalIndex, context: data, error: null }); })
      .catch((e) => { if (!cancelled) setFetched({ evalIndex, context: null, error: e.message }); });
  ...
  return (
    <Modal
      ...
      panelClassName="fixed inset-8 z-50 flex flex-col rounded-lg overflow-hidden"
      panelStyle={{ backgroundColor: "var(--sub-hull)", border: "1px solid var(--sub-rivet)" }}
      headerClassName="px-5 py-3"
      headerStyle={{ backgroundColor: "var(--sub-panel)" }}
      titleClassName="font-stencil text-sm"
      headerExtra={
        <>
