import { useStore, includedBackendIndices } from "../../store";

export function EvalHistory() {
  const evalResults = useStore((s) => s.evalResults);
  const selectedEvalIndex = useStore((s) => s.selectedEvalIndex);
  const evalIncluded = useStore((s) => s.evalIncluded);
  const selectEval = useStore((s) => s.selectEval);
  const toggleEvalIncluded = useStore((s) => s.toggleEvalIncluded);
  const removeEval = useStore((s) => s.removeEval);
  const evalLoading = useStore((s) => s.evalLoading);
  const evalError = useStore((s) => s.evalError);
  // Same count the Optimize button uses (completed evals only)
  const includedCount = useStore((s) => includedBackendIndices(s).length);

  if (evalResults.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center p-4">
        <p className="text-sm text-center" style={{ color: 'var(--sub-text-dim)' }}>
          No evaluations yet. Enter a prompt above to get started.
        </p>
      </div>
    );
  }

  return (
    <div className="flex-1 flex flex-col min-h-0">
      <div className="px-4 py-2" style={{ borderBottom: '1px solid var(--sub-rivet)' }}>
        <span className="text-xs" style={{ color: 'var(--sub-text-dim)' }}>
          {evalResults.length} evaluated, {includedCount} included
        </span>
        {evalError && <p className="alarm-text text-xs mt-1">{evalError}</p>}
      </div>
      <div className="flex-1 overflow-y-auto">
        {evalResults.map((evalResult, index) => {
          const isSelected = selectedEvalIndex === index;
          const isIncluded = evalIncluded.has(index);

          return (
            <div
              key={index}
              className={`flex items-center transition-colors group ${isSelected ? "" : "hover:bg-[var(--sub-panel)]"}`}
              style={{
                borderBottom: '1px solid var(--sub-hull)',
                backgroundColor: isSelected ? 'var(--sub-panel-light)' : undefined,
                borderLeft: isSelected ? '2px solid var(--sub-brass)' : '2px solid transparent',
              }}
            >
              {/* Checkbox: its own hit area; the rest of the row selects */}
              <label className="pl-3 py-3 flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  checked={isIncluded}
                  onChange={() => toggleEvalIncluded(index)}
                  aria-label="Include in optimization"
                  className="w-3.5 h-3.5 rounded cursor-pointer accent-amber-600"
                />
              </label>

              {/* Prompt area */}
              <button
                type="button"
                onClick={() => selectEval(index)}
                aria-current={isSelected || undefined}
                className="flex-1 text-left px-3 py-3 min-w-0"
              >
                <span className="block text-sm truncate" style={{ color: 'var(--sub-text)' }}>
                  {evalResult.prompt}
                </span>
                <span className="block text-xs mt-0.5" style={{ color: 'var(--sub-text-dim)' }}>
                  {evalResult.toolChain.length} tool call
                  {evalResult.toolChain.length !== 1 ? "s" : ""}
                </span>
              </button>

              {/* Delete */}
              <button
                onClick={() => removeEval(index)}
                disabled={evalLoading}
                className="pr-3 py-3 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus-visible:opacity-100 transition-opacity disabled:cursor-not-allowed disabled:group-hover:opacity-30"
                title={evalLoading ? "Wait for the running evaluation to finish" : "Remove evaluation"}
                aria-label="Remove evaluation"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="var(--sub-red)" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                </svg>
              </button>
            </div>
          );
        })}
      </div>
    </div>
  );
}
